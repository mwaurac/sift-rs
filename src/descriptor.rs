...
pub fn compute_descriptors(
    ...
    keypoints.iter().map(|kp| {
        ...
        let ptx = kp.x * scale_factor * 0.5; // ÷2 for the upscale
        let pty = kp.y * scale_factor * 0.5;

        let scl = kp.size * scale_factor * 0.5
            * SIFT_DESCR_SCL_FCTR
            / SIFT_DESCR_WIDTH as f32;
...
fn calc_sift_descriptor(
    ...
    for i in (-radius)..=radius {
        for j in (-radius)..=radius {
            ...
            if rbin <= -1.0 || rbin >= d as f32 { continue; }
            if cbin <= -1.0 || cbin >= d as f32 { continue; }
    ...
    let norm2: f32 = descriptor.iter().map(|&x| (x as f32).powi(2)).sum::<f32>().sqrt().max(1e-7);
    for v in &mut descriptor {
        *v = ((*v as f32 / norm2) * SIFT_INT_DESCR_FCTR).round().min(255.0) as u8;
    }
...
fn trilinear_add(
    ...
    if do_ < 0.0 { do_ += n as f32; }

    // 2×2×2 interpolation
    for (ri, rv) in [(0, 1.0 - dr), (1, dr)] {
        let r = r0 + ri;
        if r < 0 || r >= d as i32 { continue; }
        for (ci, cv) in [(0, 1.0 - dc), (1, dc)] {
            let c = c0 + ci;
            if c < 0 || c >= d as i32 { continue; }
