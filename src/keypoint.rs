            ...
            for r in SIFT_IMG_BORDER..(h - SIFT_IMG_BORDER) {
                for c in SIFT_IMG_BORDER..(w - SIFT_IMG_BORDER) {
                    ...
                    if is_extremum(val, prev, curr, next, r, c) {
                        ...
                        let refined = adjust_local_extrema(
                            dog_pyr,
                            gauss_pyr,
                            o,
                            l,
...
fn pix(img: &Image, r: usize, c: usize) -> f32 {
    img.get(r, c)
}
...
fn adjust_local_extrema(
    ...
    for _ in 0..SIFT_MAX_INTERP_STEPS {
        ...
        let dxy = 0.25
            * (curr.get(r + 1, c + 1) - curr.get(r + 1, c - 1) - curr.get(r - 1, c + 1)
                + curr.get(r - 1, c - 1));
        let dxs = 0.25
            * (next.get(r, c + 1) - next.get(r, c - 1) - prev.get(r, c + 1) + prev.get(r, c - 1));
        let dys = 0.25
            * (next.get(r + 1, c) - next.get(r - 1, c) - prev.get(r + 1, c) + prev.get(r - 1, c));

        // Solve H * x = -g using Cramer's rule on the 3×3 Hessian.
        // H = [[dxx, dxy, dxs], [dxy, dyy, dys], [dxs, dys, dss]]
        let (ox, or_, os) = solve_3x3(dxx, dxy, dxs, dxy, dyy, dys, dxs, dys, dss, -dx, -dy, -ds);
        ...
        col = (col as i32 + xc.round() as i32)
            .clamp(SIFT_IMG_BORDER as i32, img_w - SIFT_IMG_BORDER as i32 - 1)
            as usize;
        row = (row as i32 + xr.round() as i32)
            .clamp(SIFT_IMG_BORDER as i32, img_h - SIFT_IMG_BORDER as i32 - 1)
            as usize;
    ...
    let contr = (curr.get(r, c) + 0.5 * (dx * xc + dy * xr + ds * xi)) / 255.0;
    ...
    let octave_scale = (1 << octave) as f32 * 0.5;
    let kp_x = (col as f32 + xc) * octave_scale;
    let kp_y = (row as f32 + xr) * octave_scale;
    let kp_size = sigma as f32
        * 2f32.powf((layer as f32 + xi) / n_octave_layers as f32)
        * (1 << octave) as f32;

    // Pack octave, layer, and interpolated scale similarly to OpenCV.
    let interp = ((xi + 0.5) * 255.0).round().clamp(0.0, 255.0) as i32;
    let kp_octave = octave as i32 | (layer as i32) << 8 | interp << 16;
...
fn assign_orientations(
    ...
    for i in (-radius)..=radius {
        ...
        for j in (-radius)..=radius {
            ...
            let dx = img.get(y as usize, (x + 1) as usize) - img.get(y as usize, (x - 1) as usize);
            let dy = img.get((y - 1) as usize, x as usize) - img.get((y + 1) as usize, x as usize);
    ...
    let mut smoothed = vec![0f32; SIFT_ORI_HIST_BINS];
    for k in 0..SIFT_ORI_HIST_BINS {
        let hm2 = hist[(k + SIFT_ORI_HIST_BINS - 2) % SIFT_ORI_HIST_BINS];
        let hm1 = hist[(k + SIFT_ORI_HIST_BINS - 1) % SIFT_ORI_HIST_BINS];
        let hp1 = hist[(k + 1) % SIFT_ORI_HIST_BINS];
        let hp2 = hist[(k + 2) % SIFT_ORI_HIST_BINS];
        smoothed[k] =
            (hm2 + hp2) * (1.0 / 16.0) + (hm1 + hp1) * (4.0 / 16.0) + hist[k] * (6.0 / 16.0);
    }
    hist = smoothed;
    ...
    for k in 0..SIFT_ORI_HIST_BINS {
        ...
        if cur > left && cur > right && cur >= SIFT_ORI_PEAK_RATIO * max_val {
            ...
            let angle = if (angle - 360.0).abs() < 1e-4 {
                0.0
            } else {
                angle
            };
