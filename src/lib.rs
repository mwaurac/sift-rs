...
mod tests {
    ...
    fn checkerboard(cell: usize) -> Image {
        let data = (0..H).flat_map(|r| {
            (0..W).map(move |c| {
                if (r / cell + c / cell) % 2 == 0 { 200.0f32 } else { 50.0f32 }
            })
        }).collect();
        Image::new(W, H, data)
    }
    ...
    fn blob_produces_keypoints() {
        let (kps, descs) = detect_and_compute(
            &blob_image(64.0, 64.0, 10.0),
            &default_params(),
        );
        assert!(!kps.is_empty(), "Gaussian blob should produce at least one keypoint");
        assert_eq!(kps.len(), descs.len());
    }
    ...
    fn descriptors_are_not_all_zero() {
        ...
        assert!(!descs.is_empty());
        for d in &descs {
            assert!(d.iter().any(|&v| v != 0), "descriptor should not be all zeros");
        }
    }
    ...
    fn n_features_cap_is_respected() {
        let params = SiftParams { n_features: 5, ..default_params() };
